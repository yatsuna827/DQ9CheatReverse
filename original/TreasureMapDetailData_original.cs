...
namespace DQ9_Cheat.GameData
{
	internal class TreasureMapDetailData
	{
		...
		public bool IsValidRank
		{
			get
			{
				return this._validRankList.Contains(this._mapRank);
			}
		}

		public bool IsValidPlace(byte place)
		{
			return this._validPlaceList.Contains(place);
		}

		public ReadOnlyCollection<byte> ValidPlaceList
		{
			get
			{
				return this._validPlaceList.AsReadOnly();
			}
		}
		...
		public ReadOnlyCollection<byte> LowRankValidPlaceList
		{
			get
			{
				return this._lowRankValidPlaceList.AsReadOnly();
			}
		}
		...
		public ReadOnlyCollection<byte> MiddleRankValidPlaceList
		{
			get
			{
				return this._middleRankValidPlaceList.AsReadOnly();
			}
		}
		...
		public ReadOnlyCollection<byte> HighRankValidPlaceList
		{
			get
			{
				return this._highRankValidPlaceList.AsReadOnly();
			}
		}
		...
		private uint GenerateRandom()
		{
			...
			this._seed += 12345U;
			return this._seed >> 16 & 32767U;
		}

		private uint GenerateRandom(uint div)
		{
			uint num = this.GenerateRandom();
			return num % div;
		}
		...
		{
			uint num = this.GenerateRandom(100U);
			uint num2 = 0U;
			for (int i = 0; i < tableSize; i++)
			{
				num2 += (uint)table[i * 4 + 1];
				if (num < num2)
				{
					return (uint)table[i * 4];
				}
			}
			return 0U;
		}
		...
		{
			uint num = this.GenerateRandom();
			int num2 = (int)(val2 - val1 + 1);
			if (num2 == 0)
			{
				return (uint)val1;
			}
			return (uint)((ulong)val1 + (ulong)num % (ulong)((long)num2));
		}
		...
		private uint Seek4(byte[] table1, byte[] table2, int roopCount)
		{
			...
			{
				if (this.MapRank >= table1[i * 4] && this.MapRank <= table1[i * 4 + 1])
				{
					int num = 0;
					for (int j = (int)table1[i * 4 + 2]; j <= (int)table1[i * 4 + 3]; j++)
					{
						num += (int)table2[(j - 1) * 2 + 1];
					}
					int num2 = (int)((ulong)this.GenerateRandom() % (ulong)((long)num));
					num = 0;
					for (int k = (int)table1[i * 4 + 2]; k <= (int)table1[i * 4 + 3]; k++)
					{
						num += (int)table2[(k - 1) * 2 + 1];
						if (num2 < num)
						{
							return (uint)k;
						}
		...
		private uint GenerateRandom(uint value1, uint value2)
		{
			...
			uint num = this.GenerateRandom();
			int num2 = (int)(value2 - value1 + 1U);
			if (num2 == 0)
			{
				return value1;
			}
			return (uint)((ulong)value1 + (ulong)num % (ulong)((long)num2));
		}
		...
		private unsafe void CreateDungeonDetail(bool createSearchData)
		{
			...
			{
				int num2 = i - 1;
				TreasureMapDetailData._dungeonInfo[num2, 0] = (byte)i;
				TreasureMapDetailData._dungeonInfo[num2, 8] = 0;
				this._seed = (uint)((int)this.MapSeed + i);
				if (!createSearchData || i > 2)
				{
					this.routine1(num2, 792U, 1U, 256U);
					TreasureMapDetailData._dungeonInfo[num2, 21] = 1;
					TreasureMapDetailData._dungeonInfo[num2, 22] = 0;
					TreasureMapDetailData._dungeonInfo[num2, 23] = 0;
					TreasureMapDetailData._dungeonInfo[num2, 1] = 0;
					this.SetValue(num2, 24U, 1, 1, TreasureMapDetailData._dungeonInfo[num2, 2] - 2, TreasureMapDetailData._dungeonInfo[num2, 3] - 2);
					TreasureMapDetailData._dungeonInfo[num2, 28] = 0;
					TreasureMapDetailData._dungeonInfo[num2, 29] = 0;
					this.routineF(num2, 24U);
					for (int j = 0; j < (int)TreasureMapDetailData._dungeonInfo[num2, 21]; j++)
					{
						if (this.routineC(num2, (uint)(j * 12 + 24), (uint)(TreasureMapDetailData._dungeonInfo[num2, 23] * 20) + 472U))
						{
							ref byte ptr = ref TreasureMapDetailData._dungeonInfo[num2, 23];
							ptr += 1;
						}
					}
					for (int k = 0; k < (int)TreasureMapDetailData._dungeonInfo[num2, 21]; k++)
					{
						this.GenerateFloorMap(num2, (uint)(k * 12 + 24));
					}
					for (int l = 0; l < (int)TreasureMapDetailData._dungeonInfo[num2, 22]; l++)
					{
						this.routineG(num2, (uint)((l << 4) + 216));
					}
					for (int m = 0; m < (int)TreasureMapDetailData._dungeonInfo[num2, 2]; m++)
					{
						TreasureMapDetailData._dungeonInfo[num2, m + 792] = 1;
						TreasureMapDetailData._dungeonInfo[num2, ((int)(TreasureMapDetailData._dungeonInfo[num2, 3] - 1) << 4) + m + 792] = 1;
					}
					for (int n = 0; n < (int)TreasureMapDetailData._dungeonInfo[num2, 3]; n++)
					{
						TreasureMapDetailData._dungeonInfo[num2, (n << 4) + 792] = 1;
						TreasureMapDetailData._dungeonInfo[num2, (n << 4) + 792 + (int)TreasureMapDetailData._dungeonInfo[num2, 2] - 1] = 1;
					}
					this.routineJ(num2);
					if (TreasureMapDetailData._dungeonInfo[num2, 0] <= 2)
					{
						TreasureMapDetailData._dungeonInfo[num2, 8] = 0;
					}
					else
					...
					if (!createSearchData)
					{
						TreasureMapDetailData._dungeonInfo[num2, 1306] = 0;
						TreasureMapDetailData._dungeonInfo[num2, 1307] = 0;
						for (int num3 = 1; num3 < 15; num3++)
						{
							for (int num4 = 1; num4 < 15; num4++)
							{
								int num5 = (int)TreasureMapDetailData._dungeonInfo[num2, num4 + (num3 << 4) + 792];
								if (num5 != 1 && num5 != 3)
								{
									ref byte ptr2 = ref TreasureMapDetailData._dungeonInfo[num2, 1306];
									ptr2 += 1;
									num = (int)TreasureMapDetailData._dungeonInfo[num2, num4 + (num3 - 1 << 4) + 792];
									if (num != 1 && num != 3)
									{
										ref byte ptr3 = ref TreasureMapDetailData._dungeonInfo[num2, 1307];
										ptr3 += 1;
									}
									num = (int)TreasureMapDetailData._dungeonInfo[num2, (num3 << 4) + 792 + num4 - 1];
									if (num != 1 && num != 3)
									{
										ref byte ptr4 = ref TreasureMapDetailData._dungeonInfo[num2, 1307];
										ptr4 += 1;
									}
								}
							}
						}
						TreasureMapDetailData._dungeonInfo[num2, 1304] = (byte)((int)this._details[2] + (i - 1) / 4);
						int num6 = (int)((TreasureMapDataTable.TableJ[(int)(this._details[3] - 1)] * 12 + TreasureMapDetailData._dungeonInfo[num2, 1304] - 1) * 18);
						TreasureMapDetailData._dungeonInfo[num2, 1305] = TreasureMapDataTable.TableK[num6];
						TreasureMapDetailData._dungeonInfo[num2, 1312] = 0;
						fixed (byte* ptr5 = &TreasureMapDetailData._dungeonInfo[num2, 0])
						{
			...
			for (int num13 = 2; num13 < (int)this._details[1]; num13++)
			{
				this._seed = (uint)((int)this.MapSeed + num13 + 1);
				for (int num14 = 0; num14 < (int)TreasureMapDetailData._dungeonInfo[num13, 8] << 1; num14++)
				{
					this.GenerateRandom();
				}
				num = (int)this._details[2] + num13 / 4;
				for (int num15 = 0; num15 < (int)TreasureMapDetailData._dungeonInfo[num13, 8]; num15++)
				{
					TreasureMapDetailData._dungeonInfo[num13, num15 + 9] = (byte)this.GetItemRank((uint)TreasureMapDataTable.TableN[(num - 1) * 4 + 1], (uint)TreasureMapDataTable.TableN[(num - 1) * 4 + 2]);
					byte[] details = this._details2;
					byte b = TreasureMapDetailData._dungeonInfo[num13, num15 + 9] - 1;
					details[(int)b] = details[(int)b] + 1;
				}
				if (TreasureMapDetailData._dungeonInfo[num13, 8] > 0)
				{
					for (int num16 = 0; num16 < (int)TreasureMapDetailData._dungeonInfo[num13, 8]; num16++)
					{
						TreasureBoxInfo treasureBoxInfo = new TreasureBoxInfo(num16, (int)TreasureMapDetailData._dungeonInfo[num13, 9 + num16], (int)TreasureMapDetailData._dungeonInfo[num13, num16 * 2 + 13], (int)TreasureMapDetailData._dungeonInfo[num13, num16 * 2 + 14]);
						int num17 = 0;
						while (num17 < this._treasureBoxInfoList[num13].Count && treasureBoxInfo.Rank <= this._treasureBoxInfoList[num13][num17].Rank)
						{
							num17++;
						}
						this._treasureBoxInfoList[num13].Insert(num17, treasureBoxInfo);
					}
				}
			}
		}
		...
		private const uint ConstValue1 = 1103515245U;

		private const uint ConstValue2 = 12345U;
